  ...
  <ul className={cn('flex w-full flex-col gap-4', className)} {...props}>
    {links.map((link) => (
      <li key={link.id}>
        <Link
          {...linkProps(link)}
          className="flex w-full items-center gap-4 rounded-3xl border-3 border-violet-400 bg-gradient-to-br from-violet-300/50 to-violet-200/50 px-5 py-4 transition outline-none hover:border-violet-600 hover:shadow-lg hover:shadow-violet-300/50 focus-visible:ring-3 focus-visible:ring-violet-600 focus-visible:ring-offset-2 focus-visible:ring-offset-violet-100"
        >
          <Icon name={link.icon} className="h-12 w-12 shrink-0" />
          <div className="flex min-w-0 flex-col">
            <span className="truncate text-xl leading-snug font-medium">{link.text}</span>
            <span className="truncate leading-snug font-light">{link.subtext}</span>
          </div>
        </Link>
      </li>
    ))}
